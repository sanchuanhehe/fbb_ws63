...
static void app_uart_read_int_handler(const void *buffer, uint16_t length, bool error)
{
    ...
    if (buffer == NULL || length == 0) {
        printf("uart%d int mode transfer illegal data!\n", UART_BUS_0);
        return;
    }
    if (memcpy_s(g_app_uart_rx_buff, length, buffer, length) != EOK) {
        printf("uart%d int mode data copy fail!\n", UART_BUS_0);
        return;
    }
...
void *uart_task(const char *arg)
{
...
#if(UART_INT_MODE)
    printf("uart%d int mode register receive callback start!\n", UART_BUS_0);
    ...
    while (1) {
        ...
        if (uapi_uart_read(UART_BUS_0, g_app_uart_rx_buff, UART_TRANSFER_SIZE, 0)) {
            printf("uart%d poll mode receive succ!\n", UART_BUS_0);
        }
