...
static uint8_t g_sle_uart_base[] = { 0x73, 0x6c, 0x65, 0x5f, 0x73, 0x65, 0x72, 0x76, 0x65, 0x72, 0x5f, 0x74, 0x65, 0x73, 0x74 };
...
static void sle_uart_uuid_print(sle_uuid_t *uuid)
{
    ...
    if (uuid->len == UUID_16BIT_LEN) {
        ...
        printf("[%s] 0x%02x 0x%02x 0x%02x \n", __FUNCTION__, uuid->uuid[0], uuid->uuid[1],
            uuid->uuid[2], uuid->uuid[3]);
        printf("[%s] 0x%02x 0x%02x 0x%02x \n", __FUNCTION__, uuid->uuid[4], uuid->uuid[5],
            uuid->uuid[6], uuid->uuid[7]);
        printf("[%s] 0x%02x 0x%02x 0x%02x \n", __FUNCTION__, uuid->uuid[8], uuid->uuid[9],
            uuid->uuid[10], uuid->uuid[11]);
        printf("[%s] 0x%02x 0x%02x 0x%02x \n", __FUNCTION__, uuid->uuid[12], uuid->uuid[13],
    ...
    errcode_t status)
{
    printf("[%s] server_id:%d, conn_id:%d, mtu_size:%d, status:%d\r\n",
     __FUNCTION__, server_id, conn_id, mtu_size->mtu_size, status);
}
    ...
    uint16_t handle, errcode_t status)
{
    ...
    __FUNCTION__, server_id, service_handle, handle, status);
    sle_uart_uuid_print(uuid);
}
...
static errcode_t sle_server_add(void)
{
    ...
    printf("[%s] sle uart add service, server_id:%x, service_handle:%x, property_handle:%x\r\n",
        __FUNCTION__, g_server_id, g_service_handle, g_property_handle);
