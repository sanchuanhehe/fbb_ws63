...
inline void ILI9341_WR_REG(uint8_t REG)
{
    ...
    HAL_SPI_Transmit(SPI_MASTER_BUS_ID, txdata, 1, 0xff); // 不读取从机返回数据
...
inline void ILI9341_WriteRAM(uint16_t DAT)
{
    ...
    HAL_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferD, 2, 0xff);
}
/*
**********************************************************************
...
inline void ILI9341_SetCursor(uint16_t Xpos, uint16_t Ypos)
{
    ...
    HAL_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferX, 2, 0xff);

    ILI9341_WR_REG(ILI9341dev.setycmd);

    HAL_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferY, 2, 0xff);
}
/*
**********************************************************************
* @fun     :ILI9341_SetArea
...
inline void ILI9341_SetArea(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    if (x0 != old_x0 || x1 != old_x1) {
        ...
        //
        ILI9341_WR_REG(ILI9341dev.setxcmd);
        HAL_SPI_Transmit(SPI_MASTER_BUS_ID, arguments, 4, 0xff);
        //
        old_x0 = x0;
        old_x1 = x1;
    }
...
void ILI9341_Display_Dir(Screen_ShowDIR ShowDIR)
{
    ...
    ILI9341_WR_DATA(0);
    ILI9341_WR_DATA(0);
    ILI9341_WR_DATA((ILI9341dev.width - 1) >> 8);
    ILI9341_WR_DATA((ILI9341dev.width - 1) & 0XFF);
    ILI9341_WR_REG(ILI9341dev.setycmd);
    ILI9341_WR_DATA(0);
    ILI9341_WR_DATA(0);
    ILI9341_WR_DATA((ILI9341dev.height - 1) >> 8);
    ILI9341_WR_DATA((ILI9341dev.height - 1) & 0XFF);
}
/*
**********************************************************************
* @fun     :ILI9341_Init
...
void _HW_FillFrame(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color)
{
    ...
    for (i = sy; i <= ey; i++) {
        ...
        ILI9341_WriteRAM_Prepare(); // 开始写入GRAM
        for (j = 0; j < xlen; j++) {
            HAL_SPI_Transmit(SPI_MASTER_BUS_ID, TempBuffer, 2, 0xff); // 点设置颜色
        }
    }
}
...
void LCD_DrawPoint_pic(uint16_t x, uint16_t y, uint16_t color)
{
    ...
    HAL_SPI_Transmit(SPI_MASTER_BUS_ID, TempBuffer, 2, 0xff);
}
