...
static int nv_read_fun(void)
{
    ...
    if(ret == 0){
        printf("[get data]: %x  %x  %x  %x  %x  %x\r\n",read_value[0],read_value[1],read_value[2],read_value[3],read_value[4],read_value[5]);
      if( read_value[5] != 0)
      {
         buf[0]+=read_value[0];
         buf[1]+=read_value[1];
