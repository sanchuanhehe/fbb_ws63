...
void ili9341_WriteReg(uint8_t REG)
{
    ...
    hal_spi_transmit(SPI_MASTER_BUS_ID, txdata, 1, SPI_TIMEOUT_VALUE); // 不读取从机返回数据
...
void ili9341_SetArea(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    if (x0 != old_x0 || x1 != old_x1) {
        arguments[0] = x0 >> HIGH_BYTE_SHIFT;
    ...
    if (y0 != old_y0 || y1 != old_y1) {
        arguments[0] = y0 >> HIGH_BYTE_SHIFT;
        arguments[1] = y0 & LOW_BYTE_SHIFT;
