...
static void *i2c_at24c02_task(const char *arg)
{
    ...
    /* I2C data config. */
    for (uint32_t loop = 0; loop < I2C_TRANSFER_LEN; loop++) {
    ...
    while (1) {
        ...
        osal_printk("i2c%d at24c02 send start!\r\n", CONFIG_I2C_AT24C02_BUS_ID);                      //判断是否正常工作
        if (AT24C02_Write(0x00, tx_buff, 40) == ERRCODE_SUCC) {
        ...
        if (AT24C02_Read(0x00, rx_buff, 40) == ERRCODE_SUCC) {
