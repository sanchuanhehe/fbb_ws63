 ...
 *
 * Description: BREATHING_LIGHT Sample Source. \n
 *
 * History: \n
 * 2025-03-13, Create file. \n
 */
...
void generate_rainbow_colors(uint32_t *colors, uint16_t time_counter) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        switch (sector) {
            case 0: r = max_brightness; g = t;          b = p;          break;
            case 1: r = q;          g = max_brightness; b = p;          break;
            case 2: r = p;          g = max_brightness; b = t;          break;
            case 3: r = p;          g = q;          b = max_brightness; break;
            case 4: r = t;          g = p;          b = max_brightness; break;
            default: r = max_brightness; g = p;          b = q;          break;
        }

        // 限制动态亮度范围  动态亮度调整（让灯光更像呼吸/霓虹效果）
        uint8_t brightness = fmaxf(0, fminf(255, 
            max_brightness + sin(safe_counter * 0.15f + i * 0.5f) * 30
        ));

         // 颜色计算（Gamma校正）
        colors[i] = ((r * brightness) / 255) << 16 | 
                    ((g * brightness) / 255) << 8 | 
                    ((b * brightness) / 255);
    }
}
...
static void *ws2812b_task(const char *arg)
{
    ...
    while (1)
    {// 生成带有当前偏移的彩虹色
        generate_rainbow_colors(led_colors, offset);
        
        // 发送颜色到LED
        ws2812b_send_color_8(led_colors);
