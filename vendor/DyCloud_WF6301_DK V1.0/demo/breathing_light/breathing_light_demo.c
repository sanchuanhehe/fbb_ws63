...
uint16_t CalculateCurveValue(float counter) {
    // counter 是归一化时间（0 到 1）
    // 使用 sin 函数生成 S 形曲线，范围从 -1 到 1，并调整为 0 到 1
    // 公式：0.5 * (1 + sin(π * (counter - 0.5)))
    //目的：使得显示效果更佳的平滑
    float curveValue  = 0.5f * (1.0f + sinf(PI * (counter - 0.5f)));
    
    // 将 S 形曲线值映射到占空比范围 0~60000，并四舍五入为整数
    return (uint16_t)(curveValue  * g_PwmCycle + 0.5f);
}
...
static void *breathing_task(const char *arg)
{
    ...
    while (1){
        //counter禁止为0
        for (counter = 0.98; counter > 0; counter-=0.01){
        ...
        for (counter = 0.02; counter < 1; counter+=0.01){
...
static void breathing_entry(void)
{
    ...
    osal_kthread_lock();
    task_handle = osal_kthread_create((osal_kthread_handler)breathing_task, 0, "BreathingLightTask", BREATHING_LIGHT_TASK_STACK_SIZE);
    if (task_handle != NULL) {
