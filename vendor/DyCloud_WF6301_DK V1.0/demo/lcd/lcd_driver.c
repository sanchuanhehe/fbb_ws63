...
// extern bool hal_opi_set_frame_size(spi_bus_t bus, uint32_t frame_size);
...
{
    //uapi_pin_set_mode(5, CONFIG_SPI_MASTER_PIN_MODE);
...
{
    uint16_t XPOS1 = (0x00 + x1);
	uint16_t XPOS2 = (0x00 + x2);
#if SCREEN_VERSION
	uint16_t YPOS1 = (0x00 + y1);
	uint16_t YPOS2 = (0x00 + y2);
#else
    uint16_t YPOS1 = (0x0C + y1);
	uint16_t YPOS2 = (0x0C + y2);
#endif

    uint8_t lcdAreaCmds[2][4] =
	{
		{ (XPOS1 >> 8), (XPOS1 >> 0), (XPOS2 >> 8), (XPOS2 >> 0) },
		{ (YPOS1 >> 8), (YPOS1 >> 0), (YPOS2 >> 8), (YPOS2 >> 0) },
	};
    ...
    lcd_spi_data.tx_buff = lcdAreaCmds[0];
    ...
    lcd_spi_data.tx_buff = lcdAreaCmds[1];
    lcd_spi_data.tx_bytes = 4;
    LCD_DATA_MODE();
    uapi_spi_master_write(CONFIG_SPI_MASTER_BUS_ID, &lcd_spi_data, 0xFFFFFFFF);
}
...
void lcd_drv_draw_img(uint16_t x,uint16_t y,img_dsc_t *img)
{
    ...
    // osal_printk("lcd_task size %d %d %d %d !\r\n",x,x+img_w-1,y,y+img_h-1);
...
void lcd_drv_init(void)
{
    ...
    // osal_printk("lcd_spi_jd9853_cmdlist_length:%d\r\n",lcd_spi_jd9853_cmdlist_length);
}
